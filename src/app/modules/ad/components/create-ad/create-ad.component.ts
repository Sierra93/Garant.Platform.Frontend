...
export class CreateAdComponent implements OnInit, OnDestroy {
  ...
  public readonly franchiseCategoryList$ =
    this.adService.franchiseCateogryList$;

  public readonly franchiseSubcategoryList$ =
    this.adService.franchiseSubcategoryList$;
  ...
  public get canGoAhead(): boolean {
    return (
      this.createAdForm.get('franchise')?.value ||
      this.createAdForm.get('readyBusiness')?.value
    );
  }
  ...
  ngOnInit(): void {
    ...
    this.createAdForm.controls.readyBusiness.valueChanges
      .pipe(
        tap((value) => {
          if (value) {
            this.createAdForm.controls.franchise.setValue(false, {
              emitEvent: false,
            });
            this.createAdForm.controls.city.setValidators([
              Validators.required,
            ]);
    ...
    this.createAdForm.controls.franchise.valueChanges
      .pipe(
        tap((value) => {
          if (value) {
            this.createAdForm.controls.readyBusiness.setValue(false, {
              emitEvent: false,
            });
            this.createAdForm.controls.city.setValue(null, {
              emitEvent: false,
            });
  ...
  onSubmit(): void {
    const { categoryName, subCategoryName, franchise, readyBusiness, city } =
